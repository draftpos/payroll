...
def run():
    ...
    # For Havano Payroll Entry we still use the calculation
    ...
    # 1. Aggregate directly from Salary Slip custom field (INCLUDING CANCELLED)
    slips = frappe.db.sql("""
        SELECT name, employee_name, start_date, currency, docstatus, custom_total_taxable_income
        FROM `tabSalary Slip`
    ...
    for s in slips:
        ...
        # Only process the single most valid/recent slip per employee per month
        if key in processed_slips:
            continue
        processed_slips.add(key)
        
        # WE NOW JUST PULL THE FIELD DIRECTLY! No math!
        taxable = float(s.custom_total_taxable_income or 0.0)
    ...
    # 2. Aggregate missing months from Havano Payroll Entry
    ...
    print(f"Successfully processed {len(processed_slips)} valid standard slips pulling 'custom_total_taxable_income' directly!")
