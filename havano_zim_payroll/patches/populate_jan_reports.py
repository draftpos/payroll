...
def execute():
    entries = frappe.get_all("Havano Payroll Entry", fields=["name", "first_name", "last_name", "payroll_period"])
    count = 0
    
    for entry in entries:
        doc = frappe.get_doc("Havano Payroll Entry", entry.name)
        
        emps = frappe.get_all("havano_employee", filters={"first_name": doc.first_name, "last_name": doc.last_name}, limit=1)
        if not emps:
            continue
            
        emp = frappe.db.get_value("havano_employee", emps[0].name, ["name", "department", "national_id", "first_name", "last_name"], as_dict=True)
        if not emp:
            continue
        ...
        for item in items:
            ...
            if frappe.db.exists("DocType", dt_name):
                ...
                if not frappe.db.exists(dt_name, {"employee": emp.name, "payroll_period": doc.payroll_period}):
                    try:
                        frappe.get_doc({
                            "doctype": dt_name,
                            "employee": emp.name,
                        ...
                        print(f"Error inserting {dt_name}: {e}")
