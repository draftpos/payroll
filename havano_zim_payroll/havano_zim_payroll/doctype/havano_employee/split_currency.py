...
def main(self):
	...
	frappe.msgprint(
		...
		f"USD Taxable: {self.total_taxable_income_usd}<br>"
		f"ZWG Taxable: {self.total_taxable_income_zwg}<br>"
		f"Payee USD: {payee_usd}<br>"
		f"Payee ZWG: {payee_zwg}<br>"
		f"SDL USD: {sdl_usd}"
	)
	...
	# Save and link back to employee
...
def payee_against_slab_usd(amount, mode="Monthly"):
	"""Calculate PAYE based on database slabs (USD)."""
	payee = 0.0
	try:
...
def payee_against_slab_zwg(amount, mode="Monthly"):
	"""Calculate PAYE based on database slabs (ZWG)."""
	payee = 0.0
	try:
